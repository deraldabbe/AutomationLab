...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys('detroit lions', Key.RETURN);
  await driver.wait(until.titleContains('detroit lions'), 1000);
...
//await driver.executeScript("arguments[0].value = '';", searchInput);
  ...
  await driver.findElement(By.name("q")).sendkeys('superbowl', Key.RETRUN);
 //await searchInput.sendKeys("superbowl", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleContains("superbowl"), 1000);
});
